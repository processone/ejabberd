...
test_get_affiliations(Config) ->
    ...
    ?assertEqual(Nodes,
                 lists:sort([ Node || #ps_affiliation{
                                        node = Node,
                                        type = owner
                                       } <- Affs ])),
...
affiliations_master(Config) ->
    ...
    lists:foreach(
      ...
      [outcast, none, member, publish_only, publisher, owner]),
...
affiliations_slave(Config, {outcast, Node, ItemID}) ->
    ...
    #stanza_error{reason = 'forbidden'} =
        set_affiliations(Config,
                         Node,
                         [{?config(master, Config), outcast},
                          {my_jid(Config), owner}]),
...
purge_node(Config, Node) ->
    ...
    case send_recv(Config,
                   #iq{
                     type = set,
                     to = PJID,
                     sub_els = [#pubsub_owner{purge = Node}]
                    }) of
        #iq{type = result, sub_els = []} ->
            ok;
        #iq{type = error} = IQ ->
...
get_default_node_config(Config) ->
    ...
    case send_recv(Config,
                   #iq{
                     type = get,
                     to = PJID,
                     sub_els = [#pubsub_owner{default = {<<>>, undefined}}]
                    }) of
        #iq{
          type = result,
...
get_node_config(Config, Node) ->
    ...
    case send_recv(Config,
                   #iq{
                     type = get,
                     to = PJID,
                     sub_els = [#pubsub_owner{configure = {Node, undefined}}]
                    }) of
        #iq{
          type = result,
...
get_affiliations(Config, Node) ->
    ...
    case send_recv(Config,
                   #iq{
                     type = get,
                     to = PJID,
                     sub_els = [#pubsub_owner{affiliations = {Node, []}}]
                    }) of
        #iq{
          type = result,
...
get_subscriptions(Config, Node) ->
    ...
    case send_recv(Config,
                   #iq{
                     type = get,
                     to = PJID,
                     sub_els = [#pubsub_owner{subscriptions = {Node, []}}]
                    }) of
        #iq{
          type = result,
