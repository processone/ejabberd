...
init_per_testcase(TestCase, OrigConfig) ->
    ...
    Nick = if
               IsSlave -> ?config(slave_nick, OrigConfig);
               IsMaster -> ?config(master_nick, OrigConfig);
               true -> ?config(nick, OrigConfig)
           end,
    MyResource = if
                     IsMaster and IsCarbons -> MasterResource;
    ...
    Config = if
                 IsSlave -> set_opt(peer, Master, Config2);
            ...
            #iq{type = result, sub_els = []} =
                send_recv(
                  ...
                  #iq{
                    type = set,
                    sub_els = [#register{
                                 username = str:to_upper(?config(user, Config)),
                                 password = ?config(password, Config)
                                }]
                   });
        _ ->
...
unsupported_query(Config) ->
    ...
    #iq{type = error} = send_recv(Config,
                                  #iq{
                                    type = get,
                                    to = ServerJID,
                                    sub_els = [#caps{}]
...
presence_broadcast(Config) ->
    ...
    #message{
      type = chat,
      subject = [#text{lang = <<"en">>, data = <<"Welcome!">>}]
     } = recv_message(Config),
...
ldap_shared_roster_get(Config) ->
    Item = #roster_item{
             jid = jid:decode(<<"user2@ldap.localhost">>),
             name = <<"Test User 2">>,
             groups = [<<"group1">>],
             subscription = both
            },
