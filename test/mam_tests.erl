...
fake_by(Config) ->
    ...
    Fakes = lists:flatmap(
              ...
              [BareServerJID, FullServerJID, BareMyJID, FullMyJID]),
    ...
    lists:foreach(
      fun(N) ->
              ...
              Forw = ?match(#message{
                            ...
                            Forw),
              IMsg = ?match(#message{
                              to = MyJIDBare, from = Room
                             } = IMsg,
                            xmpp:get_subtag(Forw, #message{}),
                            IMsg),
...
recv_fin(Config, _, QueryID, ?NS_MAM_0 = NS, IsComplete) ->
    ...
    #mam_fin{
      ...
      complete = Complete,
      rsm = RSM
     } = xmpp:get_subtag(FinMsg, #mam_fin{xmlns = NS}),
    ct:comment("Checking if complete is ~s", [IsComplete]),
    ?match(IsComplete, Complete),
    RSM.
...
query_rsm_before(Config, From, To, NS) ->
    lists:foldl(
      ...
      <<"">>,
      lists:reverse([ lists:seq(1, N) || N <- lists:seq(0, 5) ])).
