...
-record(state, {
          subscription = none :: none | from | to | both,
...
process_subscriptions_master(Config, Actions) ->
    ...
    lists:foldl(
      fun({N, {Dir, Type}}, State) ->
              if
                  Dir == out -> put_event(Config, {N, in, Type});
...
get_items(Config, Version) ->
    case send_recv(Config,
                   #iq{
                     type = get,
                     sub_els = [#roster_query{ver = Version}]
                    }) of
        #iq{
          type = result,
