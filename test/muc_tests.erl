...
service_message_error(Config) ->
    ...
    lists:foreach(
      ...
      [chat, normal, headline, groupchat]),
    ServiceResource = jid:replace_resource(Service, p1_rand:get_string()),
...
service_improper_iq_error(Config) ->
    ...
    lists:foreach(
      ...
      [#disco_item{jid = Service},
       #identity{category = <<"category">>, type = <<"type">>},
       #vcard_email{},
       #muc_subscribe{nick = ?config(nick, Config)}]),
    disconnect(Config).
...
set_room_affiliation(Config) ->
    ...
    Body = misc:json_encode(#{
                              room => RoomName,
                              service => RoomService,
                              user => PeerJID#jid.luser,
                              host => PeerJID#jid.lserver,
                              affiliation => member
                             }),
    ...
    #message{
      ...
      to = MyJID,
      sub_els = [#muc_user{
                   items = [#muc_item{affiliation = member, role = none, jid = PeerJID}]
                  }]
     } = recv_message(Config),
...
voice_request_master(Config) ->
    ...
    ApprovalFs = muc_request:encode([{jid, PeerJID},
                                     {role, participant},
                                     {roomnick, PeerNick},
                                     {request_allow, true}]),
    send(Config,
...
change_affiliation_master(Config) ->
    ...
    lists:foreach(
      ...
      [{member, participant, available},
       {none, visitor, available},
       {admin, moderator, available},
       {owner, moderator, available},
...
config_voice_request_interval_master(Config) ->
    ...
    Fs = muc_request:encode([{jid, PeerJID},
                             {role, participant},
                             {roomnick, PeerNick},
                             {request_allow, false}]),
    send(Config,
...
get_affiliations(Config, Aff) ->
    ...
    case send_recv(Config,
                   #iq{
                     type = get,
                     to = Room,
                     sub_els = [#muc_admin{items = [#muc_item{affiliation = Aff}]}]
                    }) of
