        ...
        {ID, PID} ->
            case store_session(LUser, LServer, ID, PushJID, Node, XData) of
                {ok, _} ->
                    ...
                    ejabberd_sm:set_user_info(LUser,
                                              LServer,
                                              LResource,
                                              push_id,
                                              ID);
...
notify(LServer, PushLJID, Node, XData, Pkt0, Dir, HandleResponse) ->
    ...
    case {make_summary(LServer, Pkt, Dir), mod_push_opt:notify_on(LServer)} of
        ...
        {Summary, _NotifyOn} ->
            ...
            PubSub = #pubsub{
                       publish = #ps_publish{node = Node, items = [Item]},
    ...
    case use_cache(Mod, LServer) of
        true ->
            ...
            ets_cache:update(
              ?PUSH_CACHE,
              {LUser, LServer, ID},
              {ok, {ID, PushJID, Node, XData}},
              fun() ->
                      Mod:store_session(LUser,
                                        LServer,
                                        ID,
                                        PushJID,
                                        Node,
...
drop_online_sessions(LUser, LServer, Clients) ->
    OnlineIDs = lists:filtermap(
                  ...
                  ejabberd_sm:get_user_info(LUser, LServer)),
