...
change_password(Username,
    ...
    password_correct = check_password(Username,
                                      Host,
                                      PasswordOld),
    ok = ejabberd_auth:set_password(Username,
                                    Host,
                                    Password),
...
check_password(Username, Host, Password) ->
    case ejabberd_auth:check_password(Username,
                                      <<"">>,
                                      Host,
                                      Password) of
...
register_account(Username, Host, Password, Ip) ->
    try mod_register_opt:access(Host) of
        ...
        _:{module_not_loaded, mod_register, _Host} ->
            {error, host_unknown}
    end.


register_account2(Username, Host, Password, Ip) ->
    case mod_register:try_register(Username, Host, Password, Ip, ?MODULE) of
        ok ->
...
unregister_account(Username, Host, Password) ->
    ...
    password_correct = check_password(Username,
                                      Host,
                                      Password),
    ok = ejabberd_auth:remove_user(Username,
                                   Host,
                                   Password),
