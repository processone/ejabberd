...
handle_info({iq_reply, IQReply, blocked_domains},
            #state{
              blocked_domains = OldBlockedDomains,
              rtbl_host = RTBLHost,
              rtbl_domains_node = RTBLDomainsNode,
              host = Host
             } =
...
drop_from_cache(LJID, #state{jid_cache = Cache} = State) ->
    ...
    if
        map_size(Cache1) < map_size(Cache) ->
...
get_commands_spec() ->
    [#ejabberd_commands{
     ...
     #ejabberd_commands{
       name = drop_from_spam_filter_cache,
       tags = [spam],
