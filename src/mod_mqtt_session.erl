    ...
    case mqtt_codec:is_error_code(Code) of
        ...
        false ->
            ...
            ?DEBUG("Unexpected PUBREC with id=~B, "
                   "sending PUBREL with error code '~ts'",
                   [ID, Code1]),
...
handle_packet(#pubcomp{id = ID}, State) ->
    ?DEBUG("Ignoring unexpected PUBCOMP with id=~B: most likely "
           "it's a repeated response to duplicated PUBREL",
           [ID]),
    {ok, State};
    ...
    case maps:take(ID, State#state.acks) of
        ...
        error ->
            ...
            ?DEBUG("Unexpected PUBREL with id=~B, "
                   "sending PUBCOMP with error code '~ts'",
                   [ID, Code]),
...
set_will_properties(State,
    ...
    State#state{
      will = Will#publish{properties = Props1},
...
get_connack_properties(#state{session_expiry = SessExp, jid = JID},
    ...
    Props2#{
      session_expiry_interval => SessExp div 1000,
...
-spec subscribe([{binary(), sub_opts()}],
                jid:ljid(),
                non_neg_integer(),
                [reason_code()],
                subscriptions(),
    ...
    case mod_mqtt:subscribe(USR, TopicFilter, SubOpts, SubID) of
        ok ->
            ...
            subscribe(TopicFilters,
                      ...
                      SubID,
                      [Code | Codes],
                      maps:put(TopicFilter, {SubOpts, SubID}, Subs),
                      Err);
        {error, Why} = Err1 ->
...
store_topic_alias(State,
                  #publish{
                    topic = <<_, _/binary>> = Topic,
...
validate_will(#connect{will = undefined}, _) ->
    ok;
validate_will(#connect{
                will = #publish{topic = Topic, payload = Payload},
                will_properties = Props
               },
              JID) ->
