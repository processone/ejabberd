...
-export([start_link/0,
         stop/0,
         route/1, route/2,
         open_session/5, open_session/6, open_session/7,
         close_session/4,
         ...
         user_resources/2,
         kick_user/2, kick_user/3,
         kick_user_restuple/2,
         kick_users/1,
         get_session_pid/3,
         get_session_sid/3,
         get_session_sids/2, get_session_sids/3,
...
get_user_info(User, Server, Resource) ->
    ...
    case get_sessions(Mod, LUser, LServer, LResource) of
        ...
        Ss ->
            ...
            [{node, Node}, {ts, Ts}, {pid, Pid}, {priority, Priority} | Session#session.info]
    end.
...
-spec set_presence(sid(),
                   binary(),
                   binary(),
                   binary(),
                   prio(),
                   presence()) -> ok | {error, notfound}.
...
-spec unset_presence(sid(),
                     binary(),
                     binary(),
                     binary(),
                     binary()) -> ok | {error, notfound}.
...
-spec close_session_unset_presence(sid(),
                                   binary(),
                                   binary(),
                                   binary(),
                                   binary()) -> ok.

close_session_unset_presence(SID,
                             User,
                             Server,
                             Resource,
                             Status) ->
...
c2s_handle_info(#{lang := Lang, jid := JID} = State, kick) ->
    ...
    ejabberd_hooks:run(sm_kick_user,
                       JID#jid.lserver,
...
host_up(Host) ->
    ...
    ejabberd_hooks:add(offline_message_hook,
                       Host,
                       ejabberd_sm,
                       bounce_offline_message,
                       100),
    ejabberd_hooks:add(bounce_sm_packet,
                       Host,
                       ejabberd_sm,
                       bounce_sm_packet,
                       100),
...
host_down(Host) ->
    ...
    ejabberd_hooks:delete(offline_message_hook,
                          ...
                          bounce_offline_message,
                          100),
    ejabberd_hooks:delete(bounce_sm_packet,
                          Host,
                          ejabberd_sm,
                          bounce_sm_packet,
                          100),
...
-spec check_for_sessions_to_replace(binary(),
                                    binary(),
                                    binary(),
...
-spec check_existing_resources(binary(),
                               binary(),
                               binary(),
                               {binary(), binary()} | undefined) -> ok.
...
get_max_user_sessions(LUser, Host) ->
    case ejabberd_shaper:match(Host,
                               max_user_sessions,
...
get_commands_spec() ->
    [#ejabberd_commands{
     ...
     #ejabberd_commands{
       ...
       policy = admin,
       module = ?MODULE,
       function = connected_users_number,
       result_example = 2,
       args = [],
