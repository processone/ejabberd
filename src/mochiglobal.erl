...
get(K, Default) -> get(K, Default, key_to_module(K)).

get(_K, Default, Mod) ->
    try Mod:term() catch error:undef -> Default end.

-spec put(atom(), any()) -> ok.
%% @doc Store term V at K, replaces an existing term if present.
put(K, V) -> put(K, V, key_to_module(K)).
put(_K, V, Mod) ->
    Bin = compile(Mod, V),
    code:purge(Mod),
    code:load_binary(Mod,
...
delete(K) -> delete(K, key_to_module(K)).

delete(_K, Mod) -> code:purge(Mod), code:delete(Mod).

-spec key_to_module(atom()) -> atom().
key_to_module(K) ->
    jlib:binary_to_atom(<<"mochiglobal:",
			    (iolist_to_binary(atom_to_list(K)))/binary>>).
...
forms(Module, T) ->
    [erl_syntax:revert(X)
     || X <- term_to_abstract(Module, term, T)].
...
get_put_delete_test() ->
    K = '$$test$$mochiglobal',
    delete(K),
    ?assertEqual(bar, (get(K, bar))),
    try (?MODULE):put(K, baz),
	?assertEqual(baz, (get(K, bar))),
	(?MODULE):put(K, wibble),
	?assertEqual(wibble, ((?MODULE):get(K)))
    after
      delete(K)
    end,
    ?assertEqual(bar, (get(K, bar))),
    ?assertEqual(undefined, ((?MODULE):get(K))),
    ok.
