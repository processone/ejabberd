...
-export([init/2,
         ...
         get_nick/3,
         set_nick/4]).
-export([register_online_room/4,
         unregister_online_room/4,
         find_online_room/3,
         get_online_rooms/3,
         count_online_rooms/2,
         rsm_supported/0,
...
-export([set_affiliation/6,
         set_affiliations/4,
         get_affiliation/5,
         get_affiliations/3,
         search_affiliation/4]).
                        ...
                        Allow = case mnesia:select(
                                    ...
                                    [] when (ServiceOrRoom == Service) ->
                                        ...
                                        not lists:any(fun({_, {_NRUS, NRServiceOrRoom}, _Nick}) ->
                                                              Service == (jid:decode(NRServiceOrRoom))#jid.lserver
                                                      end,
                                                      NickRegistrations);
                                    [] -> true;
...
transform({muc_room, {N, H}, Opts} = R)
  when is_list(N) orelse is_list(H) ->
    R#muc_room{
      name_host = {iolist_to_binary(N), iolist_to_binary(H)},
...
transform(#muc_registered{us_host = {{U, S}, H}, nick = Nick} = R) ->
    R#muc_registered{
      us_host = {{iolist_to_binary(U), iolist_to_binary(S)},
                 iolist_to_binary(H)},
