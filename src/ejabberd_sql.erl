...
-export([start_link/2,
         ...
         sql_query_t/1,
         sql_transaction/2, sql_transaction/4,
         sql_bloc/2, sql_bloc/3,
         abort/1,
         restart/1,
         use_new_schema/0,
         sql_query_to_iolist/1, sql_query_to_iolist/2,
         escape/1,
         standard_escape/1,
         escape_like/1,
         escape_like_arg/1,
...
-type state() :: #state{}.
-type sql_query_simple(T) :: [sql_query(T) | binary()] |
                             binary() |
                             #sql_query{} |
                             fun(() -> T) |
                             fun((atom(), _) -> T).
...
sync_send_event(Proc, Msg, Timeout) ->
    try
        p1_fsm:sync_send_event(Proc, Msg, Timeout)
    catch
        _:{Reason, {p1_fsm, _, _}} ->
            {error, Reason}
    end.
...
decode_term(Bin) ->
    ...
    try
        ...
        _:{badmatch, {error, {Line, Mod, Reason}, _}} ->
...
init([Host]) ->
    ...
    case ejabberd_option:sql_keepalive_interval(Host) of
        undefined ->
            ok;
        KeepaliveInterval ->
            timer:apply_interval(KeepaliveInterval,
                                 ?MODULE,
                                 keep_alive,
                                 [Host, self()])
    end,
...
connecting({sql_cmd, {sql_query, ?KEEPALIVE_QUERY}, Timestamp},
    ...
    {next_state, connecting, State};
connecting({sql_cmd, Command, Timestamp} = Req,
           From,
           State) ->
    ...
    PendingRequests =
        try
            p1_queue:in({sql_cmd, Command, From, Timestamp},
                        State#state.pending_requests)
        catch
            error:full ->
...
session_established({sql_cmd, Command, Timestamp},
                    From,
                    State) ->
...
sql_query_internal(Query) ->
    ...
    Res = case State#state.db_type of
              odbc ->
                  to_odbc(odbc:sql_query(State#state.db_ref,
                                         [Query],
                                         QueryTimeout - 1000));
              mssql ->
                  to_odbc(odbc:sql_query(State#state.db_ref,
                                         ...
                                         QueryTimeout - 1000));
              pgsql ->
                  pgsql_to_odbc(pgsql:squery(State#state.db_ref,
                                             Query,
                                             QueryTimeout - 1000));
              mysql ->
...
generic_escape() ->
    #sql_escape{
      string = fun(X) -> <<"'", (escape(X))/binary, "'">> end,
...
pgsql_escape() ->
    #sql_escape{
      string = fun(X) -> <<"E'", (escape(X))/binary, "'">> end,
      integer = fun(X) -> misc:i2l(X) end,
      boolean = fun(true) -> <<"'t'">>;
                   (false) -> <<"'f'">>
                end,
...
sqlite_escape() ->
    #sql_escape{
      string = fun(X) -> <<"'", (standard_escape(X))/binary, "'">> end,
      integer = fun(X) -> misc:i2l(X) end,
      boolean = fun(true) -> <<"1">>;
                   (false) -> <<"0">>
                end,
...
pgsql_prepare(SQLQuery, State) ->
    ...
    {RArgs, _} =
        lists:foldl(
          ...
          {[], 1},
          (SQLQuery#sql_query.args)(Escape)),
...
pgsql_item_to_odbc({<<"SELECT", _/binary>>,
                    Rows,
                    Recs}) ->
    {selected, [ element(1, Row) || Row <- Rows ], Recs};
pgsql_item_to_odbc({<<"FETCH", _/binary>>,
                    Rows,
                    Recs}) ->
    {selected, [ element(1, Row) || Row <- Rows ], Recs};
...
mysql_connect(Server, Port, DB, Username, Password, ConnectTimeout, Transport, SSLOpts0) ->
    ...
    case p1_mysql_conn:start(binary_to_list(Server),
                             Port,
    ...
    case filelib:ensure_dir(odbcinst_config()) of
        ok ->
            try
                ...
                error:{badmatch, {error, Reason} = Err} ->
