...
-export([iq_sm/1,
         ...
         process_pubsub/1,
         process_pubsub_owner/1,
         process_vcard/1,
         process_commands/1]).

%% exports for console debug manual use
-export([create_node/5, create_node/7,
         delete_node/3,
         subscribe_node/5,
         unsubscribe_node/5,
         publish_item/6, publish_item/8,
         delete_item/4, delete_item/5,
         send_items/7,
         get_items/2,
         get_item/3,
         get_cached_item/2,
...
-export([err_closed_node/0,
         err_configuration_required/0,
         err_invalid_jid/0,
         err_invalid_options/0,
         err_invalid_payload/0,
         err_invalid_subid/0,
         err_item_forbidden/0,
         err_item_required/0,
         err_jid_required/0,
         err_max_items_exceeded/0,
         err_max_nodes_exceeded/0,
         err_nodeid_required/0,
         err_not_in_roster_group/0,
         err_not_subscribed/0,
         err_payload_too_big/0,
         err_payload_required/0,
         err_pending_subscription/0,
         err_precondition_not_met/0,
         err_presence_subscription_required/0,
         err_subid_required/0,
         err_too_many_subscriptions/0,
         err_unsupported/1,
...
init([ServerHost | _]) ->
    ...
    AllPlugins =
        lists:flatmap(
                  ...
                  lists:foreach(
                    fun(H) ->
                            ...
                            try
                                ...
                                error:badarg when H == ServerHost ->
                                    ok
                            end
                    end,
                    [Host, ServerHost]),
          ...
          Hosts),
    ejabberd_hooks:add(c2s_self_presence,
                       ServerHost,
                       ?MODULE,
                       on_self_presence,
                       75),
    ejabberd_hooks:add(c2s_terminated,
                       ServerHost,
                       ?MODULE,
                       on_user_offline,
                       75),
    ...
    ejabberd_hooks:add(disco_local_features,
                       ...
                       75),
    ejabberd_hooks:add(disco_local_items,
                       ServerHost,
                       ?MODULE,
                       disco_local_items,
                       75),
    ejabberd_hooks:add(presence_probe_hook,
                       ServerHost,
                       ?MODULE,
                       presence_probe,
                       80),
    ...
    ejabberd_hooks:add(remove_user,
                       ...
                       remove_user,
                       50),
    ejabberd_hooks:add(c2s_handle_info,
                       ServerHost,
                       ?MODULE,
                       c2s_handle_info,
                       50),
    case lists:member(?PEPNODE, AllPlugins) of
        true ->
            ejabberd_hooks:add(caps_add,
                               ServerHost,
                               ?MODULE,
                               caps_add,
                               80),
            ejabberd_hooks:add(caps_update,
                               ServerHost,
                               ?MODULE,
                               caps_update,
                               80),
            ...
            ejabberd_hooks:add(disco_sm_features,
                               ...
                               disco_sm_features,
                               75),
            ejabberd_hooks:add(disco_sm_items,
                               ServerHost,
                               ?MODULE,
                               disco_sm_items,
                               75),
...
depends(ServerHost, Opts) ->
    ...
    lists:flatmap(
      fun(Name) ->
              ...
              try
                  apply(Plugin, depends, [Host, ServerHost, Opts])
              catch
                  _:undef -> []
              end
      end,
...
-spec disco_local_identity([identity()],
                           jid(),
                           jid(),
                           binary(),
                           binary()) -> [identity()].
...
-spec disco_sm_identity([identity()],
                        jid(),
                        jid(),
                        binary(),
                        binary()) -> [identity()].
    ...
    Action =
        fun(#pubsub_node{
              ...
              owners = O
                ...
                case get_allowed_items_call(Host,
                                            Nidx,
                                            From,
                                            Type,
                                            Options,
    ...
    lists:foreach(
      ...
      lists:usort(
        lists:foldl(
          ...
          [],
          [Entity#jid.lserver, Owner#jid.lserver]))).
                  ...
                  {result, Subs} ->
                      lists:foreach(
                        fun({#pubsub_node{
                             ...
                             _,
                             JID}) ->
                                Unsubscribe = match_option(Options, access_model, presence) andalso
                                    lists:member(BJID, node_owners_action(Host, PType, Nidx, O)),
                                case Unsubscribe of
                                    true ->
                                        node_action(Host,
                                                    PType,
...
terminate(_Reason,
    ...
    case lists:member(?PEPNODE, Plugins) of
        true ->
            ejabberd_hooks:delete(caps_add,
                                  ServerHost,
                                  ?MODULE,
                                  caps_add,
                                  80),
            ejabberd_hooks:delete(caps_update,
                                  ServerHost,
                                  ?MODULE,
                                  caps_update,
                                  80),
            ...
            ejabberd_hooks:delete(disco_sm_features,
                                  ...
                                  disco_sm_features,
                                  75),
            ejabberd_hooks:delete(disco_sm_items,
                                  ServerHost,
                                  ?MODULE,
                                  disco_sm_items,
                                  75),
    ...
    ejabberd_hooks:delete(c2s_self_presence,
                          ServerHost,
                          ?MODULE,
                          on_self_presence,
                          75),
    ejabberd_hooks:delete(c2s_terminated,
                          ServerHost,
                          ?MODULE,
                          on_user_offline,
                          75),
    ...
    ejabberd_hooks:delete(disco_local_features,
                          ...
                          disco_local_features,
                          75),
    ejabberd_hooks:delete(disco_local_items,
                          ServerHost,
                          ?MODULE,
                          disco_local_items,
                          75),
    ejabberd_hooks:delete(presence_probe_hook,
                          ServerHost,
                          ?MODULE,
                          presence_probe,
                          80),
    ...
    ejabberd_hooks:delete(remove_user,
                          ...
                          remove_user,
                          50),
    ejabberd_hooks:delete(c2s_handle_info,
                          ServerHost,
                          ?MODULE,
                          c2s_handle_info,
                          50),
...
iq_pubsub(Host,
    ...
    case {IQType, SubEl} of
        ...
        {set, #pubsub{
                publish = #ps_publish{node = Node, items = Items},
        ...
        {set, #pubsub{
                retract = #ps_retract{node = Node, notify = Notify, items = Items},
                _ = undefined
               }} ->
            case Items of
                [#ps_item{id = ItemId}] ->
        ...
        {set, #pubsub{
                subscribe = #ps_subscribe{node = Node, jid = JID},
                options = Options,
                _ = undefined
               }} ->
        ...
        {set, #pubsub{
                unsubscribe = #ps_unsubscribe{node = Node, jid = JID, subid = SubId},
                _ = undefined
               }} ->
        ...
        {get, #pubsub{
                options = #ps_options{node = Node, subid = SubId, jid = JID},
                _ = undefined
               }} ->
...
-spec send_pending_node_form(binary(),
                             jid(),
                             binary(),
                             [binary()]) -> adhoc_command() | {error, stanza_error()}.
send_pending_node_form(Host, Owner, Lang, Plugins) ->
...
-spec send_pending_auth_events(binary(),
                               binary(),
                               jid(),
...
send_authorization_request(#pubsub_node{
                             nodeid = {Host, Node},
                             type = Type,
                             id = Nidx,
                             owners = O
...
-spec handle_authorization_response(binary(),
                                    message(),
...
-spec create_node(host(),
                  ...
                  binary(),
                  atom(),
                  [{binary(), [binary()]}]) -> {result, pubsub()} | {error, stanza_error()}.
...
create_node(Host, ServerHost, Node, Owner, GivenType, Access, Configuration) ->
        ...
        fun() ->
                ...
                case node_call(Host,
                    ...
                    {result, true} ->
                        case tree_call(Host,
                                       create_node,
                                       [Host, Node, Type, Owner, NodeOptions, Parents]) of
                            {ok, Nidx} ->
    ...
    case transaction(Host, CreateNode, transaction) of
        {result, {Nidx, SubsByDepth, {Result, broadcast}}} ->
            ...
            ejabberd_hooks:run(pubsub_create_node,
                               ServerHost,
...
subscribe_node(Host, Node, From, JID, Configuration) ->
    ...
    case transaction(Host, Node, Action, sync_dirty) of
        {result, {TNode, {Result, subscribed, SubId, send_last}}} ->
            ...
            ServerHost = serverhost(Host),
            ejabberd_hooks:run(pubsub_subscribe_node,
                               ServerHost,
...
unsubscribe_node(Host, Node, From, JID, SubId) ->
    ...
    case transaction(Host, Node, Action, sync_dirty) of
        {result, {_, default}} ->
            ServerHost = serverhost(Host),
            ejabberd_hooks:run(pubsub_unsubscribe_node,
                               ServerHost,
                               [ServerHost, Host, Node, Subscriber, SubId]),
            {result, undefined};
        Error -> Error
...
publish_item(Host, ServerHost, Node, Publisher, ItemId, Payload, PubOpts, Access) ->
    ...
    case transaction(Host, Node, Action, sync_dirty) of
        {result, {TNode, {Result, Broadcast, Removed}}} ->
            ...
            ejabberd_hooks:run(pubsub_publish_item,
                               ServerHost,
...
delete_item(Host, Node, Publisher, ItemId, ForceNotify) ->
    ...
    case transaction(Host, Node, Action, sync_dirty) of
        {result, {TNode, {Result, broadcast}}} ->
            ...
            Options = TNode#pubsub_node.options,
            ServerHost = serverhost(Host),
            ejabberd_hooks:run(pubsub_delete_item,
                               ServerHost,
...
get_items(Host, Node, From, SubId, _MaxItems, ItemIds, RSM) ->
                        ...
                        case ItemIds of
                            [ItemId] ->
                                ...
                                case node_call(Host,
                                               Type,
                                               get_item,
                                               [Nidx, ItemId, From, AccessModel, PS, RG, undefined]) of
    ...
    case transaction(Host, Node, Action, sync_dirty) of
        {result, {TNode, {Items, RsmOut}}} ->
            ...
            Options = TNode#pubsub_node.options,
            {result, #pubsub{
                       items = items_els(Node, Options, SendItems),
                       rsm = RsmOut
                      }};
        {result, {TNode, Item}} ->
            Options = TNode#pubsub_node.options,
...
-spec get_allowed_items_call(host(),
                             nodeIdx(),
                             jid(),
                             binary(),
                             nodeOptions(),
                             [ljid()]) -> {result, [#pubsub_item{}]} |
                                          {error, stanza_error()}.
...
-spec get_options_helper(binary(),
                         jid(),
                         binary(),
                         binary(),
                         _,
...
-spec set_options_helper(binary(),
                         [{binary(), [binary()]}],
                         jid(),
                         nodeIdx(),
                         binary(),
...
get_subscriptions(Host, Node, JID, Plugins) when is_list(Plugins) ->
    ...
    case Result of
        {ok, Subs} ->
            Entities = lists:flatmap(fun({#pubsub_node{nodeid = {_, SubsNode}}, Sub}) ->
                                             case Node of
                                                 <<>> ->
                                                     [#ps_subscription{
...
-spec get_presence_and_roster_permissions(host(),
                                          jid(),
                                          [ljid()],
                                          accessModel(),
...
get_presence_and_roster_permissions(Host, From, Owners, AccessModel, AllowedGroups) ->
    if
        (AccessModel == presence) or (AccessModel == roster) ->
            case Host of
                {User, Server, _} ->
...
is_to_deliver(LJID, NotifyType, Depth, NodeOptions, SubOptions) ->
    sub_to_deliver(LJID, NotifyType, Depth, SubOptions) andalso
    ...
    Els = case get_option(Options, itemreply) of
              publisher ->
                  [ #ps_item{id = ItemId, sub_els = Payload, publisher = jid:encode(USR)}
                    || #pubsub_item{itemid = {ItemId, _}, payload = Payload, modification = {_, USR}} <- Items ];
              _ ->
                  [ #ps_item{id = ItemId, sub_els = Payload}
...
-spec broadcast_publish_item(host(),
                             binary(),
                             nodeIdx(),
                             binary(),
                             nodeOptions(),
...
-spec broadcast_retract_items(host(),
                              jid(),
                              binary(),
                              nodeIdx(),
                              binary(),
                              nodeOptions(),
                              [itemId()]) -> {result, boolean()}.
...
-spec broadcast_removed_node(host(),
                             binary(),
                             nodeIdx(),
                             binary(),
                             nodeOptions(),
                             subs_by_depth()) -> {result, boolean()}.
...
-spec broadcast_created_node(host(),
                             binary(),
                             nodeIdx(),
                             binary(),
                             nodeOptions(),
...
-spec broadcast_config_notification(host(),
                                    binary(),
                                    nodeIdx(),
                                    binary(),
                                    nodeOptions(),
                                    binary()) -> {result, boolean()}.
...
-spec get_options_for_subs(host(),
                           nodeIdx(),
...
send_items(Host, Node, Nidx, Type, Options, Publisher, SubLJID, ToLJID, Number) ->
    ...
    case Items of
        ...
        Items ->
            ...
            Stanza = #message{
                       sub_els = [#ps_event{items = items_els(Node, Options, Items)} | Delay]
                      },
            NotificationType = get_option(Options, notification_type, headline),
            send_stanza(Publisher,
                        ToLJID,
                        Node,
                                                                           ...
                                                                           case lists:member(JIDToDeliver, JIDs) of
                                                                               ...
                                                                               false ->
                                                                                   ...
                                                                                   {[JIDToDeliver | JIDsAcc],
                                                                                    [{JIDToDeliver, NodeName, [SubID]} | RecipientsAcc]};
                                                                               true ->
...
decode_subscribe_options(#xdata{fields = Fs}, Lang) ->
    try
        pubsub_subscribe_options:decode(Fs)
    catch
...
decode_publish_options(#xdata{fields = Fs}, Lang) ->
    try
        pubsub_publish_options:decode(Fs)
    catch
...
tree_action(Host, Function, Args) ->
    ...
    Fun = fun() ->
                  try
                      ...
                      ?EX_RULE(Class, Reason, St) when DBType == sql ->
                          StackTrace = ?EX_STACK(St),
...
do_transaction(ServerHost, Fun, Trans, DBType) ->
    F = fun() ->
                try
                    ...
                    ?EX_RULE(Class, Reason, St) when (DBType == mnesia andalso
                       ...
                       case lists:member(<<"retrieve-affiliations">>, plugin_features(Host, Type)) of
                           false ->
                               {{error, extended_error(xmpp:err_feature_not_implemented(),
                                ...
                                Acc};
                           true ->
                               Items = lists:member(<<"retract-items">>, Features) andalso
        ...
        {ok, Affs} ->
            lists:foreach(
              fun({Node, Affiliation}) ->
                      ...
                      Purge = (get_option(Options, purge_offline) andalso
                               get_option(Options, persist_items)),
                      if
                          (Publisher or Open) and Purge ->
...
get_commands_spec() ->
    [#ejabberd_commands{
     ...
     #ejabberd_commands{
       name = delete_expired_pubsub_items,
       tags = [purge],
