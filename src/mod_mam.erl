...
-export([sm_receive_packet/1, user_receive_packet/1, user_send_packet/1,
	 ...
	 get_room_config/4, set_room_option/3, offline_message/1, export/1, delete_user_old_messages/4]).
...
-callback delete_user_old_messages(binary() | global,
						erlang:timestamp(),
...
delete_user_old_messages(TypeBin, Days, User, Host) when TypeBin == <<"chat">>;
	TypeBin == <<"groupchat">>;
	TypeBin == <<"all">> ->
	CurrentTime = p1_time_compat:system_time(micro_seconds),
	Diff = Days * 24 * 60 * 60 * 1000000,
	...
	Mod = gen_mod:db_mod(DBType, ?MODULE),
	Mod:delete_user_old_messages(Host, TimeStamp, Type, User);
delete_user_old_messages(_TypeBin, _Days, _User, _Host) ->
	unsupported_type.
...
get_commands_spec() ->
    [
			#ejabberd_commands{name = delete_old_mam_messages, tags = [purge],
				...
				result = {res, rescode}},
			#ejabberd_commands{name = delete_user_old_mam_messages, tags = [purge],
				desc = "Delete user MAM messages older than DAYS",
				longdesc = "Valid message TYPEs: "
				"\"chat\", \"groupchat\", \"all\".",
				module = ?MODULE, function = delete_user_old_messages,
