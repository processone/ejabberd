...
-spec select_with_mucsub(binary(),
                         jid(),
                         jid(),
                         mam_query:result(),
                         #rsm_set{} | undefined,
...
do_select_query(LServer,
    ...
    case QRes of
        {{selected, _, Res}, {selected, _, [[Count]]}} ->
            ...
            {Res1, IsComplete} =
                if
                    Max >= 0 andalso Max /= undefined andalso length(Res) > Max ->
                        if
                            Direction == before ->
            ...
            {lists:flatmap(
               fun([TS, XML, PeerBin, Kind, Nick]) ->
                       case make_archive_el(JidArchiveS,
                                            TS,
                                            XML,
                                            PeerBin,
                                            Kind,
                  ...
                  ([User, TS, XML, PeerBin, Kind, Nick]) ->
                       case make_archive_el(User,
                                            TS,
                                            XML,
                                            PeerBin,
                                            Kind,
                                            Nick,
                                            {groupchat, member, MucState},
                                            JidRequestor,
                                            jid:decode(User)) of
                           {ok, El} ->
...
make_sql_query(User, LServer, MAMQuery, RSM, ExtraUsernames) ->
    ...
    LimitClause = if
                      is_integer(Max), Max >= 0, ODBCType /= mssql ->
                               ...
                               subscribers_table ->
                                   ...
                                   {<<" username,">>,
                                    [<<" (username = ">>,
                                     ...
                                     RoomName,
                                     <<" from muc_room_subscribers where jid=">>,
                                     SJid,
                                     HostMatch,
                                     <<"))">>]};
                               _ ->
    ...
    QueryPage =
        case Direction of
            before ->
                ...
                [<<"SELECT">>,
                 UserSel,
                 <<" timestamp, xml, peer, kind, nick FROM (">>,
                 Query,
                 SubOrderClause,
                 LimitClause,
    ...
    {QueryPage,
     [<<"SELECT COUNT(*) FROM archive WHERE ">>,
      UserWhere,
      HostMatch,
      WithClause,
      WithTextClause,
