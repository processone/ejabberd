                          ...
                          _ ->
                              case eval_file(FilePath) of
                                  {ok, Data} ->
                                      ...
                                      convert_data(misc:uri_decode(Host),
                                                   Type,
                                                   misc:uri_decode(Name),
                                                   Data);
                                  Err ->
                                      Err
                              end
...
convert_data(_Host, "config", _User, [Data]) ->
    ...
    case proplists:get_bool(<<"persistent">>, Config) of
        true when RoomJID /= error ->
            mod_muc:store_room(find_serverhost(RoomJID#jid.lserver),
                               RoomJID#jid.lserver,
                               RoomJID#jid.luser,
                               RoomCfg);
        _ ->
...
convert_privacy_item({_, Item}) ->
    ...
    MatchAll = if
                   (MatchIQ == false) and (MatchMsg == false) and
