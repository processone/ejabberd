...
-export([start_link/0,
         ...
         import_dir/1,
         %% Purge DB
         delete_expired_messages/0,
         delete_old_messages/1,
         %% Mnesia
         get_master/0,
         set_master/1,
         backup_mnesia/1,
...
get_commands_spec() ->
    [
     ...
     #ejabberd_commands{
       name = join_cluster_here,
       tags = [cluster],
       desc = "Join a remote Node here, into our cluster",
       note = "added in 24.06",
       module = ?MODULE,
       function = join_cluster_here,
     ...
     #ejabberd_commands{
       name = list_cluster_detailed,
       tags = [cluster],
       desc = "List nodes (both running and known) and some stats",
       note = "added in 24.06",
       module = ?MODULE,
       function = list_cluster_detailed,
       args = [],
     ...
     #ejabberd_commands{
       name = delete_old_messages_batch,
       tags = [offline, purge],
       desc = "Delete offline messages older than DAYS",
       note = "added in 22.05",
       module = ?MODULE,
     ...
     #ejabberd_commands{
       name = delete_old_messages_status,
       tags = [offline, purge],
     ...
     #ejabberd_commands{
       name = abort_delete_old_messages,
       tags = [offline, purge],
     ...
     #ejabberd_commands{
       ...
       args = [{nodename, string}],
       result = {res, restuple}
      },
     #ejabberd_commands{
       name = mnesia_change_nodename,
       tags = [mnesia],
       desc = "Change the erlang node name in a backup file",
       module = ?MODULE,
       function = mnesia_change_nodename,
     ...
     #ejabberd_commands{
       ...
       module = ?MODULE,
       function = mnesia_info,
       args = [],
       result = {res, string}
      },
     #ejabberd_commands{
       name = mnesia_table_info,
       tags = [mnesia],
       desc = "Dump info on Mnesia table state",
       module = ?MODULE,
       function = mnesia_table_info,
     ...
     #ejabberd_commands{
       ...
       longdesc = "The binary backup file is "
                  ...
                  "_`restore`_ API.",
       module = ?MODULE,
       function = install_fallback_mnesia,
     ...
     #ejabberd_commands{
       name = mnesia_table_details,
       tags = [internal, mnesia],
       desc = "Get details of a Mnesia table",
       module = ?MODULE,
       function = mnesia_table_details,
       args = [{table, binary}],
     ...
     #ejabberd_commands{
       name = mnesia_table_change_storage,
       tags = [mnesia],
       ...
       module = ?MODULE,
       function = mnesia_table_change_storage,
       args = [{table, binary}, {storage_type, binary}],
       result = {res, restuple}
      },
     #ejabberd_commands{
       name = mnesia_table_clear,
       tags = [internal, mnesia],
       desc = "Delete all content in a Mnesia table",
       module = ?MODULE,
       function = mnesia_table_clear,
       args = [{table, binary}],
       result = {res, restuple}
      },
     #ejabberd_commands{
       name = mnesia_table_destroy,
       tags = [internal, mnesia],
       desc = "Destroy a Mnesia table",
       module = ?MODULE,
       function = mnesia_table_destroy,
       args = [{table, binary}],
       result = {res, restuple}
      },
     #ejabberd_commands{
...
delete_old_messages_batch(Server, Days, BatchSize, Rate) ->
    ...
    case ejabberd_batch:register_task({spool, LServer},
                                      0,
                                      Rate,
                                      {LServer, Days, BatchSize, none},
                                      fun({L, Da, B, IS} = S) ->
