...
-spec handle_call(_, {pid(), _}, state()) ->
          {reply, {ok, pos_integer(),
                       ...
                       pos_integer() | undefined},
                  state()} |
          {reply, {error, atom()}, state()} |
          {noreply, state()}.
handle_call({use_slot, Slot, Size},
            _From,
            #state{
              file_mode = FileMode,
              dir_mode = DirMode,
...
process_iq(#iq{type = get, lang = Lang, sub_els = [#disco_info{}]} = IQ,
    ...
    AddInfo = ejabberd_hooks:run_fold(disco_info,
                                      ServerHost,
                                      [],
                                      [ServerHost, ?MODULE, <<"">>, <<"">>]),
...
create_slot(#state{
    ...
    case ejabberd_hooks:run_fold(http_upload_slot_request,
                                 ServerHost,
                                 allow,
...
create_slot(#state{service_url = ServiceURL},
    ...
    GetRequest = <<ServiceURL/binary,
                   "?jid=",
                   (misc:url_encode(JidStr))/binary,
                   "&name=",
                   (misc:url_encode(File))/binary,
                   "&size=",
                   (misc:url_encode(SizeStr))/binary,
                   "&content_type=",
                   (misc:url_encode(ContentType))/binary>>,
    case httpc:request(get,
                       {binary_to_list(GetRequest), []},
                       HttpOptions,
                       Options) of
...
-spec encode_addr(inet:ip_address() |
    ...
    Form = case mod_http_upload_opt:max_size(Host) of
               ...
               MaxSize ->
                   lists:foldl(
                     ...
                     AddInfo,
                     [?NS_HTTP_UPLOAD_0, ?NS_HTTP_UPLOAD])
           end,
    #disco_info{
      identities = [#identity{
                      category = <<"store">>,
...
-spec do_store_file(file:filename_all(),
                    http_request(),
                    integer() | undefined,
                    integer() | undefined) ->
          ok | {error, term()}.
