...
-spec disco_local_identity([identity()], jid(), jid(), binary(), binary()) ->
                              [identity()].
disco_local_identity(Acc, _From, _To, Node, Lang) ->
    case tokenize(Node) of
        [<<"api-commands">>] ->
            ?INFO_COMMAND(?T("API Commands"), Lang);
        [<<"api-commands">>, CommandName] ->
            ?INFO_COMMAND(get_api_command_desc(CommandName), Lang);
        _ ->
            Acc
    end.

get_api_command_desc(NameAtom) ->
    iolist_to_binary((get_api_command(NameAtom))#ejabberd_commands.desc).
...
get_api_commands(From, Server, Lang) ->
    lists:map(fun({Name, _Args, _Desc}) ->
              ...
              ejabberd_commands:list_commands(?DEFAULT_VERSION, get_caller_info(From))).
...
get_api_command(Name) when is_binary(Name) ->
    get_api_command(binary_to_existing_atom(Name, latin1));
get_api_command(Name) when is_atom(Name) ->
    ejabberd_commands:get_command_definition(Name).
...
get_form_api_command(NameBin, Host, _Lang) ->
    ...
    FieldsArgs =
        build_fields(Def#ejabberd_commands.args,
                     ...
                     [{host, Host}, {service, mod_muc_admin:find_hosts(Host)}],
                     true),
    FieldsArgsWithHeads =
        case FieldsArgs of
            [] ->
...
set_form_api_command(From, Host, CommandNameBin, XData, _Lang) ->
    %% Description
    Def = get_api_command(CommandNameBin),
    Title = list_to_binary(atom_to_list(Def#ejabberd_commands.name)),
    Instructions = get_instructions(Def),
