...
process_mix_create(#iq{
                     to = To,
                     from = From,
                     sub_els = [#mix_create{channel = Chan, xmlns = XmlNs}]
                    } = IQ) ->
    Host = To#jid.lserver,
    ServerHost = ejabberd_router:host_of_route(Host),
    Mod = gen_mod:db_mod(ServerHost, ?MODULE),
...
process_mix_destroy(#iq{
                      ...
                      sub_els = [#mix_destroy{channel = Chan, xmlns = XmlNs}]
                     } = IQ) ->
    Host = To#jid.lserver,
    ServerHost = ejabberd_router:host_of_route(Host),
    Mod = gen_mod:db_mod(ServerHost, ?MODULE),
...
process_mix_join(#iq{
                   to = To,
                   from = From,
                   sub_els = [#mix_join{xmlns = XmlNs} = JoinReq]
                  } = IQ) ->
    Chan = To#jid.luser,
    Host = To#jid.lserver,
    ServerHost = ejabberd_router:host_of_route(Host),
...
process_mix_setnick(#iq{
                      to = To,
                      from = From,
                      sub_els = [#mix_setnick{nick = Nick, xmlns = XmlNs}]
                     } = IQ) ->
    {Chan, Host, _} = jid:tolower(To),
    ServerHost = ejabberd_router:host_of_route(Host),
    Mod = gen_mod:db_mod(ServerHost, ?MODULE),
            ...
            case Mod:get_participant(ServerHost, Chan, Host, BFrom) of
                {ok, {StableID, Nick}} ->
                    ...
                    Msg1 = xmpp:set_subtag(
                             Msg#message{
                               from = jid:replace_resource(To, StableID),
                               to = undefined,
...
-spec notify_participant_joined(module(),
                                binary(),
                                jid(),
                                jid(),
                                binary(),
...
notify_participant_joined(Mod, LServer, To, From, ID, Nick) ->
    {Chan, Host, _} = jid:tolower(To),
    Participant = #mix_participant{
                    jid = jid:remove_resource(From),
                    nick = Nick
                   },
    Item = #ps_item{
             id = ID,
