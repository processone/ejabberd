...
init([Host]) ->
    ...
    ejabberd_hooks:add(local_send_to_resource_hook,
                       ...
                       50),
    ejabberd_hooks:add(muc_filter_presence,
                       Host,
                       ?MODULE,
                       muc_presence_filter,
                       50),
...
terminate(_Reason, #rtbl_state{host = Host, subscribed = Sub, retry_timer = Timer}) ->
    ejabberd_hooks:delete(local_send_to_resource_hook,
                          ...
                          pubsub_event_handler,
                          50),
    ejabberd_hooks:delete(muc_filter_presence,
                          Host,
                          ?MODULE,
                          muc_presence_filter,
                          50),
...
request_initial_items(Host) ->
    IQ = #iq{
           ...
           to = jid:make(mod_muc_rtbl_opt:rtbl_server(Host)),
           sub_els = [#pubsub{items = #ps_items{node = mod_muc_rtbl_opt:rtbl_node(Host)}}]
          },
    ...
    case xmpp:get_subtag(IQ, #pubsub{}) of
        #pubsub{items = #ps_items{node = Node, items = Items}} ->
            ...
            SubIQ = #iq{
                      ...
                      from = To,
                      to = From,
                      sub_els = [#pubsub{subscribe = #ps_subscribe{jid = To, node = Node}}]
                     },
...
pubsub_event_handler(#message{
                       from = #jid{luser = <<>>, lserver = SServer},
                       to = #jid{
                              luser = <<>>,
                              lserver = Server,
                              lresource = <<"rtbl-", _/binary>>
                             }
                      } = Msg) ->
    ...
    lists:foreach(
      fun(CHost) ->
              lists:foreach(
                ...
                mod_muc:get_online_rooms(CHost))
      end,
