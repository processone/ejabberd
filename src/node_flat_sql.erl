                ...
                MaxItems == unlimited ->
                    ...
                    case get_item(Nidx, ItemId) of
                        ...
                        {result, _} ->
                            {error, xmpp:err_forbidden()};
                        _ ->
                            OldIds = maybe_remove_extra_items(Nidx,
                                                              MaxItems,
                                                              GenKey,
                                                              ItemId),
...
get_item(Nidx, ItemId) ->
    case catch ejabberd_sql:sql_query_t(
                 ?SQL("select @(itemid)s, @(publisher)s, @(creation)s,"
                      ...
                      " where nodeid=%(Nidx)d and itemid=%(ItemId)s")) of
        {selected, [RItem]} ->
...
del_items(Nidx, ItemIds) ->
    ...
    SNidx = misc:i2l(Nidx),
    catch ejabberd_sql:sql_query_t([<<"delete from pubsub_item where itemid in (">>,
                                    I,
                                    <<") and nodeid='">>,
                                    SNidx,
                                    <<"';">>]).
...
itemids(Nidx) ->
    case catch ejabberd_sql:sql_query_t(
                 ?SQL("select @(itemid)s from pubsub_item where "
                      "nodeid=%(Nidx)d order by modification desc")) of
        {selected, RItems} ->
...
select_affiliation_subscriptions(Nidx, JID) ->
    ...
    case catch ejabberd_sql:sql_query_t(
                 ?SQL("select @(affiliation)s, @(subscriptions)s from "
                      " pubsub_state where nodeid=%(Nidx)d and jid=%(J)s")) of
        {selected, [{A, S}]} ->
...
select_affiliation_subscriptions(Nidx, GenKey, SubKey) ->
    ...
    case catch ejabberd_sql:sql_query_t(
                 ?SQL("select @(jid)s, @(affiliation)s, @(subscriptions)s from "
                      " pubsub_state where nodeid=%(Nidx)d and jid in (%(GJ)s, %(SJ)s)")) of
        {selected, Res} ->
            lists:foldr(
