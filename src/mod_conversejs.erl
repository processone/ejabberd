...
process([], #request{method = 'GET', host = Host, q = Query, raw_path = RawPath1}) ->
    ...
    Script = get_file_url(Host,
                          conversejs_script,
                          <<RawPath/binary, "/converse.min.js">>,
                          <<"https://cdn.conversejs.org/dist/converse.min.js">>),
    CSS = get_file_url(Host,
                       conversejs_css,
    ...
    Init = [{<<"discover_connection_methods">>, false},
            ...
            {<<"view_mode">>, <<"fullscreen">>} | ExtraOptions],
    Init2 =
    ...
    {200,
     ...
     [<<"<!DOCTYPE html>">>,
      ...
      <<"<script src='">>,
      fxml:crypt(Script),
      <<"' charset='utf-8'></script>">>] ++ PluginsHtml ++ [<<"</head>">>,
                                                            <<"<body>">>,
                                                            <<"<script>">>,
                                                            <<"converse.initialize(">>,
                                                            misc:json_encode(Init4),
                                                            <<");">>,
                                                            <<"</script>">>,
                                                            <<"</body>">>,
                                                            <<"</html>">>]};
...
get_autologin_options(Query) ->
    case {proplists:get_value(<<"autologinjid">>, Query),
          proplists:get_value(<<"autologintoken">>, Query)} of
        {undefined, _} ->
            [];
        {Jid, Token} ->
            [{<<"auto_login">>, <<"true">>},
