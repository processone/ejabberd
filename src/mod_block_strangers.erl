...
-export([filter_packet/1,
         filter_offline_msg/1,
         filter_subscription/2,
         get_sm_features/5]).
...
filter_packet({#message{from = From} = Msg, State} = Acc) ->
    ...
    case (?SETS):is_element(LFrom, PresA) orelse
         (?SETS):is_element(LBFrom, PresA) orelse
         sets_bare_member(LBFrom, PresA) of
        false ->
...
need_check(Pkt) ->
    ...
    not (IsSelf orelse IsEmpty orelse IsError orelse
         acl:match_rule(LServer, Access, From) == allow orelse
         ((AllowLocalUsers orelse From#jid.luser == <<"">>) andalso
          ejabberd_router:is_my_host(From#jid.lserver))).
...
check_subscription(From, To) ->
    ...
    case mod_roster:is_subscribed(From, To) of
        ...
        false ->
            ...
            mod_block_strangers_opt:allow_transports(LocalServer) andalso
            mod_roster:is_subscribed(jid:make(RemoteServer), To);
        true ->
            true
    end.
