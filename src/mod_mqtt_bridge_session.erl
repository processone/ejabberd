...
start(Proc, Transport, Host, Port, Path, Publish, Subscribe, Authentication, ReplicationUser) ->
    p1_server:start({local, Proc},
                    ?MODULE,
                    [Proc, Transport, Host, Port, Path, Publish, Subscribe, Authentication,
                     ReplicationUser],
                    []).


start_link(Proc, Transport, Host, Port, Path, Publish, Subscribe, Authentication, ReplicationUser) ->
    p1_server:start_link({local, Proc},
                         ?MODULE,
                         [Proc, Transport, Host, Port, Path, Publish, Subscribe,
...
handle_info({Tag, TCPSock, TCPData},
            #state{
              ws_codec = {inith, Hash, Upgrade, Auth, Last},
              socket = {Transport, _}
             } = State)
  when (Tag == tcp orelse Tag == ssl) ->
    Data = <<Last/binary, TCPData/binary>>,
...
connect({ok, Sock}, State0, Transport, Auth) ->
    ...
    Connect = case Auth of
                  {User, Pass} ->
                      #connect{
                        client_id = integer_to_binary(State#state.id),
                        clean_start = true,
                        username = User,
                        password = Pass,
                        keep_alive = 60,
...
handle_publish(#publish{topic = Topic, payload = Payload, properties = Props},
    ...
    case maps:get(Topic, Subs, none) of
        ...
        LocalTopic ->
            ...
            mod_mqtt:publish(USR,
                             #publish{retain = true, topic = LocalTopic, payload = Payload, properties = Props},
                             ExpiryTime),
            {ok, State}
    end.
