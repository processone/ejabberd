...
handle_info({route_trusted, Destinations, Packet},
    ...
    case catch route_trusted(LServiceS,
                             LServerS,
                             From,
                             Destinations,
                             Packet) of
...
report_not_jid(From, Packet, Addresses) ->
    lists:foreach(
      fun(Address) ->
              route_error(
                xmpp:set_from_to(Packet, From, From),
                jid_malformed,
...
group_by_destinations(Addrs, Map) ->
    lists:foldl(
      fun(#address{type = Type, jid = #jid{lserver = Server}} = Addr, Map2) when Type == to; Type == cc ->
...
process_discoinfo_result2(From,
                          FromS,
                          LServiceS,
...
purge_loop(NM) ->
    receive
        purge_now ->
            purge(),
            timer:send_after(?CACHE_PURGE_TIMER,
                             ?PURGE_PROCNAME,
                             purge_now),
            purge_loop(NM);
...
iq_disco_info_extras(From, State) ->
    ...
    case iq_disco_info_extras2(SenderT, Service_limits) of
        ...
        List_limits_xmpp ->
            [#xdata{
               type = result,
               fields = [?RFIELDT(hidden, <<"FORM_TYPE">>, ?NS_ADDRESS) | List_limits_xmpp]
              }]
    end.
