...
create_node(NodeIdx, Owner) ->
    ...
    set_state(#pubsub_state{stateid = {OwnerKey, NodeIdx},
        nodeidx = NodeIdx, affiliation = owner}),
    {result, {default, broadcast}}.
...
get_states(NodeIdx) ->
    States = case catch mnesia:index_read(pubsub_state,
					  NodeIdx, #pubsub_state.nodeidx)
		 of
...
get_items(NodeIdx, _From) ->
    ...
    {result,
     lists:reverse(lists:keysort(#pubsub_item.modification, Items))}.
