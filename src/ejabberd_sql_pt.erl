...
parse1([], Acc, State) ->
    ...
    State1#state{
      'query' = lists:reverse(State1#state.'query'),
...
parse1([$@, $( | S], Acc, State) ->
    ...
    Convert =
        case Type of
            integer ->
                erl_syntax:if_expr([erl_syntax:clause(
    ...
    State4 = State3#state{
               res_pos = State3#state.res_pos + 1,
            ...
            host ->
                ...
                case State#state.use_new_schema of
                    true ->
                        ...
                        State3#state{
                          'query' = [{var, Var, Type},
                                     {str, "server_host="} | State3#state.'query'],
...
parse1("%ESCAPE" ++ S, Acc, State) ->
    ...
    State2 =
        State1#state{
          'query' = [{var, Var, string} | State1#state.'query'],
