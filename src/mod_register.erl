...
-export([start/2,
         ...
         try_register/4, try_register/5,
         process_iq/1,
         send_registration_notifications/3,
         mod_opt_type/1,
         mod_options/1,
         depends/2,
         format_error/1,
...
process_iq(#iq{
           ...
           Source,
           IsCaptchaEnabled,
           _AllowRemove) when is_binary(User),
                              is_binary(Password) ->
    Server = To#jid.lserver,
...
process_iq(#iq{
             ...
             to = To,
             lang = Lang,
             sub_els = [#register{xdata = #xdata{} = X}]
            } = IQ,
           Source,
           true,
           _AllowRemove) ->
...
process_iq(#iq{type = get, from = From, to = To, id = ID, lang = Lang} = IQ,
    ...
    Instr = translate:translate(
              Lang,
              ?T("Choose a username and password to register "
                 "with this server")),
    URL = mod_register_opt:redirect_url(Server),
    if
        (URL /= undefined) and not IsRegistered ->
...
try_register_or_set_password(User,
                             Server,
                             Password,
