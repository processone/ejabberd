...
-export([tls_options/1,
         ...
         inline_stream_features/1,
         handle_sasl2_inline/2,
         handle_sasl2_inline_post/3,
         handle_bind2_inline/2,
         handle_bind2_inline_post/3,
         sasl_options/1,
         handle_sasl2_task_next/4,
         handle_sasl2_task_data/3,
...
-export([handle_unexpected_cast/2,
         ...
         process_info/2]).
%% API
-export([get_presence/1,
         set_presence/2,
         resend_presence/1, resend_presence/2,
         open_session/1,
         call/3,
         cast/2,
         send/2,
...
host_up(Host) ->
    ...
    ejabberd_hooks:add(c2s_unauthenticated_packet,
                       Host,
                       ?MODULE,
    ...
    ejabberd_hooks:add(c2s_handle_info,
                       ...
                       process_info,
                       100),
    ejabberd_hooks:add(c2s_auth_result,
                       Host,
                       ?MODULE,
                       process_auth_result,
                       100),
    ejabberd_hooks:add(c2s_handle_cast,
                       Host,
                       ?MODULE,
                       handle_unexpected_cast,
                       100),
    ejabberd_hooks:add(c2s_handle_call,
                       Host,
                       ?MODULE,
                       handle_unexpected_call,
                       100).
...
host_down(Host) ->
    ...
    ejabberd_hooks:delete(c2s_unauthenticated_packet,
                          Host,
                          ?MODULE,
                          reject_unauthenticated_packet,
                          100),
    ejabberd_hooks:delete(c2s_handle_info,
                          ...
                          process_info,
                          100),
    ejabberd_hooks:delete(c2s_auth_result,
                          Host,
                          ?MODULE,
                          process_auth_result,
                          100),
    ejabberd_hooks:delete(c2s_handle_cast,
                          Host,
                          ?MODULE,
                          handle_unexpected_cast,
                          100),
    ejabberd_hooks:delete(c2s_handle_call,
                          Host,
                          ?MODULE,
                          handle_unexpected_call,
                          100).
...
process_terminated(#{sid := SID, jid := JID, user := U, server := S, resource := R} = State,
    ...
    State1 = case maps:is_key(pres_last, State) of
                 true ->
                     ejabberd_sm:close_session_unset_presence(SID,
                                                              U,
                                                              S,
                                                              R,
                                                              Status),
...
handle_authenticated_packet(Pkt, #{lserver := LServer} = State) when not ?is_stanza(Pkt) ->
    ejabberd_hooks:run_fold(c2s_authenticated_packet,
                            ...
                            State,
                            [Pkt]);
handle_authenticated_packet(Pkt,
                            #{
                              lserver := LServer,
                              jid := JID,
                              ip := {IP, _}
...
handle_cdata(Data, #{lserver := LServer} = State) ->
    ejabberd_hooks:run_fold(c2s_handle_cdata,
                            LServer,
                            State,
                            [Data]).
...
handle_sasl2_inline(Els, #{lserver := LServer} = State) ->
    ejabberd_hooks:run_fold(c2s_handle_sasl2_inline,
                            LServer,
                            {State, Els, []},
                            []).
...
handle_sasl2_inline_post(Els, Results, #{lserver := LServer} = State) ->
    ejabberd_hooks:run_fold(c2s_handle_sasl2_inline_post,
                            LServer,
                            State,
                            [Els, Results]).
...
handle_bind2_inline(Els, #{lserver := LServer} = State) ->
    ejabberd_hooks:run_fold(c2s_handle_bind2_inline,
                            LServer,
                            {State, Els, []},
                            []).
...
handle_bind2_inline_post(Els, Results, #{lserver := LServer} = State) ->
    ejabberd_hooks:run_fold(c2s_handle_bind2_inline_post,
                            LServer,
                            State,
                            [Els, Results]).
...
handle_sasl2_task_next(Task, Els, InlineEls, #{lserver := LServer} = State) ->
    ejabberd_hooks:run_fold(c2s_handle_sasl2_task_next,
                            LServer,
                            {abort, State},
                            [Task, Els, InlineEls]).
...
handle_sasl2_task_data(Els, InlineEls, #{lserver := LServer} = State) ->
    ejabberd_hooks:run_fold(c2s_handle_sasl2_task_data,
                            LServer,
                            {abort, State},
                            [Els, InlineEls]).
        ...
        allow ->
            ...
            if
                LBareTo /= LBareFrom ->
                    ...
                    if
                        Subscription /= both andalso Subscription /= from ->
                            A = case Type of
...
broadcast_presence_unavailable(#{jid := JID, pres_a := PresA} = State,
                               Pres,
                               BroadcastToRoster) ->
    ...
    Items1 = case BroadcastToRoster of
                 true ->
                     Roster = ejabberd_hooks:run_fold(roster_get,
                                                      LServer,
                                                      [],
                                                      [{LUser, LServer}]),
                     lists:foldl(
...
change_shaper(#{
    ...
    JID = jid:make(U, S, R),
    Shaper = ejabberd_shaper:match(LServer,
                                   ShaperName,
