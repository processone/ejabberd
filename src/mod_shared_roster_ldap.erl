...
init([Host | _]) ->
    ...
    ejabberd_hooks:add(roster_get_jid_info,
                       ...
                       get_jid_info,
                       70),
    ejabberd_hooks:add(roster_process_item,
                       Host,
                       ?MODULE,
                       process_item,
                       50),
...
terminate(_Reason, State) ->
    ...
    ejabberd_hooks:delete(roster_get_jid_info,
                          ...
                          get_jid_info,
                          70),
    ejabberd_hooks:delete(roster_process_item,
                          Host,
                          ?MODULE,
                          process_item,
                          50).
...
search_user_displayed_groups(State, User) ->
    ...
    Entries = eldap_search(State,
                           [eldap_filter:do_sub(State#state.rfilter,
                                                [{<<"%u">>, User}])],
                           [GroupAttr]),
    Reply = lists:flatmap(fun(#eldap_entry{
                                attributes =
                                    Attrs
                               }) ->
                                  case Attrs of
...
search_group_info(State, Group) ->
    ...
    case eldap_search(State,
                      [eldap_filter:do_sub(State#state.gfilter,
                                           [{<<"%g">>, Group}])],
                      [State#state.group_attr,
                       State#state.group_desc,
                       State#state.uid]) of
        [] ->
            error;
        LDAPEntries ->
