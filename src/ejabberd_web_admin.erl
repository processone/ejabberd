...
process2([<<"logout">> | _], #request{lang = Lang}) ->
    Text = [?XCT(<<"h1">>, ?T("Logged Out")),
            ?XE(<<"p">>,
                [?C(<<"Your web browser has logout from WebAdmin. Close this window, or login again in: ">>),
...
get_auth_account2(HostOfRule,
    ...
    case ejabberd_auth:check_password(User, <<"">>, Server, Pass) of
        true ->
            case any_rules_allowed(HostOfRule,
                                   AccessRule,
...
make_xhtml(Els, Host, Username, Request, JID, Level)
  when (Username == unspecified) or (is_binary(Username)) ->
    ...
    WelcomeText =
        ...
        [?XAE(<<"blockquote">>,
              ...
              [?XC(<<"p">>, <<"Welcome to ejabberd's WebAdmin!">>),
               ?XC(<<"p">>,
                   <<"Browse the menu to navigate your XMPP virtual hosts, "
                     "Erlang nodes, and other global server pages...">>),
               ?XC(<<"p">>,
                   <<"Some pages have a link in the top right corner "
               ...
               ?XE(<<"p">>,
                   [?C(<<"Many pages use ejabberd's API commands to show information "
                    ...
                    ?AC(<<"https://docs.ejabberd.im/developer/ejabberd-api/">>,
                        <<"ejabberd Docs: API">>)]),
               ?XC(<<"p">>,
                   <<"For example, this is the 'stats' command, "
...
process_admin(Host, #request{path = [], lang = Lang} = R, AJID) ->
    make_xhtml([?XCT(<<"h1">>, ?T("Administration")),
                ?XE(<<"ul">>,
                    [ ?LI([?ACT(MIU, MIN)])
                      || {MIU, MIN} <- get_menu_items(Host, cluster, Lang, AJID, 2) ])],
               Host,
               R,
               AJID,
               2);
...
user_info(User, Server, #request{q = Query, lang = Lang} = R) ->
    ...
    Lasts = case gen_mod:is_loaded(Server, mod_last) of
                true ->
                    [make_command(get_last,
                                  R,
                                  [{<<"user">>, User}, {<<"host">>, Server}],
                                  []),
                     make_command(set_last,
                                  R,
                                  [{<<"user">>, User}, {<<"host">>, Server}],
                                  [])];
                false ->
    ...
    [?XAE(<<"form">>,
          [{<<"action">>, <<"">>}, {<<"method">>, <<"post">>}],
          ([make_command(user_sessions_info,
                         R,
                         [{<<"user">>, User}, {<<"host">>, Server}],
...
histogram([], _Integral, _Current, Count, Hist) ->
    if
        Count > 0 -> lists:reverse([Count | Hist]);
...
get_node(Host, Node, _NPath, Request) ->
    Res = case Host of
              global ->
                  ejabberd_hooks:run_fold(webadmin_page_node,
                                          Host,
                                          [],
                                          [Node, Request]);
              _ ->
                  ejabberd_hooks:run_fold(webadmin_page_hostnode,
                                          Host,
                                          [],
                                          [Host, Node, Request])
          end,
...
pretty_string_int(String) when is_binary(String) ->
    {_, Result} = lists:foldl(fun(NewNumber, {3, Result}) ->
                              ...
                              {0, <<"">>},
                              lists:reverse(binary_to_list(String))),
    Result.
...
make_navigation_menu(Host, Node, Username, Lang, JID, Level) ->
    HostNodeMenu = make_host_node_menu(Host,
                                       Node,
                                       Lang,
                                       JID,
                                       Level),
    HostUserMenu = make_host_user_menu(Host,
                                       Username,
                                       Lang,
                                       JID,
                                       Level),
    HostMenu = make_host_menu(Host,
                              HostNodeMenu,
                              HostUserMenu,
                              Lang,
                              JID,
...
get_menu_items_hook({hostnode, Host, Node}, Lang) ->
    ejabberd_hooks:run_fold(webadmin_menu_hostnode,
                            Host,
                            [],
                            [Host, Node, Lang]);
get_menu_items_hook({hostuser, Host, Username}, Lang) ->
    ejabberd_hooks:run_fold(webadmin_menu_hostuser,
                            Host,
                            [],
                            [Host, Username, Lang]);
get_menu_items_hook({host, Host}, Lang) ->
    ejabberd_hooks:run_fold(webadmin_menu_host,
                            Host,
                            [],
                            [Host, Lang]);
get_menu_items_hook({node, Node}, Lang) ->
...
make_menu_items2(Lang,
                 Deep,
                 {MURI, MName, [Item | Items]},
                 Res) ->
    Res2 = case Item of
               {IURI, IName} ->
                   [make_menu_item(item,
...
-spec make_command(Name :: atom(),
          ...
          xmlel() | {xmlcdata, binary()} | {raw_and_value, any(), xmlel()}
              when Option :: {only, presentation | without_presentation | button | result | value | raw_and_value} |
...
make_command2(Name, Request, BaseArguments, Options) ->
    ...
    try {ejabberd_commands:get_command_definition(Name),
         ejabberd_access_permissions:can_access(Name, CallerInfo)} of
        {C, allow} ->
            make_command2(Name,
                          Request,
                          CallerInfo,
...
execute_command(Name,
                ...
                InputNameAppend) ->
    try
        Args = prepare_arguments(Name, BaseArguments ++ Query, ArgumentsFormat),
        {Args,
...
prepare_arguments(ComName, Args, ArgsFormat) ->
    lists:foldl(fun({ArgName, ArgFormat}, FinalArguments) ->
                        ...
                        case {lists:keyfind(
                              ...
                              lists:keyfind(long_this([ComName, ArgName]), 1, Args)} of
...
make_command_result_element(ArgumentsUsed,
    ...
    ContentElements =
        [ list_to_tuple([ make_result(format_result(V, {ElementName, ElementFormat}),
                                      ...
                                      ArgumentsUsed,
                                      ResultLinks)
                          || {V, {ElementName, ElementFormat}} <- lists:zip(tuple_to_list(Tuple), TupleElements) ])
          || Tuple <- ListOfTuples ],
         ...
         ?XE(<<"tbody">>,
             [?XE(<<"tr">>,
                  [ ?XC(<<"td">>, format_result(V, {ElementName, ElementFormat}))
                    || {V, {ElementName, ElementFormat}} <- lists:zip(tuple_to_list(Values), TupleElements) ])])]);
make_command_result_element(ArgumentsUsed,
                            Value,
