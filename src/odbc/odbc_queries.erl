...
-export([get_db_type/0, update_t/4, sql_transaction/2,
	 ...
	 set_roster_version/2, add_roomhistory_sql/6,
	 clear_and_add_roomhistory/3, load_roomhistory/2]).
...
add_roomhistory_sql(Room, Nick, Packet, HaveSubject,
		    Timestamp, Size) ->
    [<<"insert into room_history(room, nick, "
       "packet, have_subject, timestamp, size) "
       "values (">>,
     quote(Room), <<",">>, quote(Nick), <<",">>,
     quote(Packet), <<",">>, HaveSubject, <<",">>, Timestamp,
     <<",">>, Size, <<");">>].

clear_and_add_roomhistory(LServer, Room, Queries) ->
    Q = [<<"delete from room_history where room "
	   "= '">>,
	 Room, <<"';">>],
    ejabberd_odbc:sql_transaction(LServer, [Q | Queries]).

load_roomhistory(LServer, Room) ->
    Q = [<<"select nick, packet, have_subject, timestamp, "
	   "size from room_history where room = "
	   "'">>,
	 Room, <<"' order by timestamp ;">>],
