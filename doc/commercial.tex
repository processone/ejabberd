...
\makesubsection{modack}{\modack{}}
\ind{modules!\modack{}}

This module takes care about reliable message delivery.
The server sends acknowledgement messages to a sender reporting
the status of a message being sent. The acknowledgement message is a message
possessing element qualified by the 'urn:xmpp:receipts' namespace (\xepref{0184}).
If a sender chooses to include \verb|<request/>| element qualified by this namespace
it should be prepared to receive the following acknowledgement messages:
\begin{itemize}
\item \verb|<on-sender-server/>| A message has been received by the sender's server.
\item \verb|<offline/>| A message has been stored in offline storage.
\item \verb|<delayed/>| A message is queued for futher delivery.
\item \verb|<received/>| A message has been successfully received by the recipient.
\item \verb|<unreliable/>| A message has been sent to the recipient and it's status
is unknown.
\end{itemize}

In the case when a message recipient doesn't support \xepref{0184} (this is checked
using it's capabilities (\xepref{0115})), \XMPP{} pings (\xepref{0199}) are used
to check the connection status of the recipient after every sent message.
Since the latter procedure is not very effective, clients are encouraged
to support \xepref{0184}.

\makesubsection{modadminp}{\modadminp{}}
\ind{modules!\modadminp{}}

This module provides additional administration commands.
Use \verb|ejabberdctl help| to get the detailed information about the commands.
...
\makesubsection{modapplepush}{\modapplepush{}}
\ind{modules!\modapplepush{}}

This module allows user connected using Apple devices to receive badges,
sounds or custom text alerts via
\footahref{http://en.wikipedia.org/wiki/Apple\_Push\_Notification\_Service}
{Apple Push Notification Service}. The module partially depends on \modapplepushservice{}
module and should know where it resides.

Options:
\begin{description}
\iqdiscitem{Push Packet (\ns{p1:push})}
\titem{\{push\_services, [ \{ AppID, ServiceName\}, ... ]\}} Specifies an application
identifier and an \XMPP{} service route of \modapplepushservice{} module.
\titem{\{default\_services, [ \{ Host, ServiceName\}, ... ]\}} Specifies a list
of default \XMPP{} service routes of \modapplepushservice{} module per virtual host.
This list is used when an application identifier doesn't match any routes.
\titem{\{default\_service, ServiceName\}} Specifies the default service route of
\modapplepushservice{} module. This option is used as a last resort.
The default is ``applepush.localhost''.
\titem{\{backend, sql|mnesia\}} Which database backend to use for caching.
The default is \term{mnesia}.
\end{description}
...
\makesubsection{modapplepushservice}{\modapplepushservice{}}
\ind{modules!\modapplepushservice{}}

This module maintains connections to
\footahref{http://en.wikipedia.org/wiki/Apple\_Push\_Notification\_Service}{APNS}
server and is used by \modapplepush{} module.

Options:
\begin{description}
\titem{\{hosts, [\{ Host, Options\}, ...]\}} Specifies a list of virtual hosts and
the corresponding options. The available options are:
\begin{description}
  \titem{\{gateway, Name\}} Specifies a domain name of the APNS server. The default is
  ``gateway.push.apple.com''.
  \titem{\{port, Integer\}} Specifies a TCP port of the gateway (APNS server, see above).
  The default is 2195.
  \titem{\{certfile, Path\}} Specifies a path to file containing PEM-encoded SSL certificate.
  \titem{\{sound\_file, Name\}} Specifies a name of a sound file a client will use
  for push notification alerts. The default is ``pushalert.wav''. Note that this name
  is used by a client only and there are no sound files stored in a server.
  \titem{\{feedback, Name\}} Specifies a domain name of the feedback server.
  The feedback server is used to receive a reason of failed push notifications.
  \titem{\{feedback\_port, Integer\}} Specifies a TCP port of the feedback server (see above).
  The default is 2196.
  \titem{\{failure\_script, Path\}} Specifies a script (shell-executed command) which
  is triggered when an SSL connection to APNS server failed. The script should accept a single
  variable - a name of the APNS server.
\end{description}
\end{description}
...
{modules,
 [
  ...
  {mod_applepush_service, [{hosts, [{"applepush.localhost",
                                     [{certfile, "ssl.pem"},
                                      {gateway, "localhost"},
                                      {port, 5555},
...
needed because all nodes exchange information about connected users, MUC
rooms, registered services, etc\ldots

\ejabberd{} uses \footahref{http://en.wikipedia.org/wiki/Consistent\_hashing}{consistent
hashing} approach to store data across the nodes in the cluster. For instance,
when a C2S session or a MUC room is created, the corresponding Erlang process
is created and a reference pointing to this process is stored in the corresponding
node. When another process wishes to send a message to this
process, it routes the message to the node holding it's reference. This node
performs a lookup for the reference and, if found, routes the message
torwards the referenced process. Note that in general the referenced process might not reside
on the same node. Thus, another internode round-trip is needed to reach the
referenced process.

Such approach allows \ejabberd{} to get rid of data replication and thus to be more
effective in internode communications. Node (dis)connects are handled in the following
manner:
\begin{itemize}
\item When some node goes down, it gets removed from the hashing on other nodes.
  Due to nature of consistent hasing, no further procedures are needed to be done.
\item When some node goes up, a rebalancing procedure is performed on other nodes.
  \begin{itemize}
  \item References are duplicated to other nodes according to the new hash values.
    This process is limited in time and is controlled by \term{rehash\_timeout} option.
    The default value is 30 seconds.
  \item As the second phase, some processes are copied (migrated) to other nodes
    according to the new hash value. This is limited as well and can be configured
    by \term{migrate\_timeout} option. The default is 120 seconds. The smaller
    timeout forces the processes to migrate faster. So, it is not recommended
    to set it to very small values in order to avoid an avalanche effect.
  \end{itemize}
\end{itemize}

Note however that some of these techniques can be changed in the future releases.
