...
process_iq_get(From, To, #iq{sub_el = SubEl} = IQ) ->
    ...
    try
		...
		IQ#iq{type = result, sub_el = case {ItemsToSend, VersionToSend} of
						 ...
						 {Items, Version} -> [{xmlel, <<"query">>, [{<<"xmlns">>, ?NS_ROSTER}, {<<"ver">>, Version}], Items}]
						end}
    catch
    	_:_ ->
...
process_item_els(Item, [{xmlel, Name, Attrs, SEls} | Els]) ->
    case Name of
	<<"group">> ->
	...
	_ ->
		...
		_ ->
		    XEls = [{xmlel, Name, Attrs, SEls} | Item#roster.xs],
